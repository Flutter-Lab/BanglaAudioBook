const bookList = [
  {
    'title': 'ডোপামিন ডিটক্স',
    'image':
        'https://drive.usercontent.google.com/download?id=1k_zviPl3pKEsWttmOLuy20du0E-Hstyj&export=download',
    'audio_src':
        'https://drive.usercontent.google.com/download?id=1RmgVBm4aQPkuxN1_mEmI47gg7xQvOLOQ&export=download&authuser=2',
    'chapter_list': [
      {
        'title': 'লেখক/পাঠক পরিচয়',
        'start_time': 1,
      },
      {
        'title': 'প্রাক-কথন',
        'start_time': 53,
      },
      {
        'title': 'ভূমিকা',
        'start_time': 252,
      },
      {
        'title': 'পার্ট-১ [ডোপামিন এবং এটি যেভাবে কাজ করে]',
        'start_time': 465,
      },
      {
        'title': 'পার্ট-২ [সমস্যা]',
        'start_time': 734,
      },
      {
        'title': 'পার্ট-৩ [ডিটক্সের উপকারিতা]',
        'start_time': 2463,
      },
      {
        'title': 'পার্ট-৪ [সফল ডোপামিন ডিটক্সের জন্য তিনটি ধাপ]',
        'start_time': 2812,
      },
      {
        'title': 'পার্ট-৫ [কাজ করুন এবং গড়িমসি করাকে জয় করুন]',
        'start_time': 3639,
      },
      {
        'title': 'পার্ট-৬ ["ডোপামিন রিল্যাপ্স" থেকে বাঁচুন]',
        'start_time': 4547,
      },
      {
        'title': 'উপসংহার',
        'start_time': 5295,
      },
    ]
  },
  {
    'title': 'দ্যা পাওয়ার অব পজিটিভ থিংকিং',
    'image':
        'https://fs.pbs.com.bd/DIR/Com/PBS/Product/ImageDetails/2201446.jpg',
    'audio_src':
        'https://drive.usercontent.google.com/uc?id=1uB98fwaTweZyzsxJTUKnuC8A-TX7757l&export=download&authuser=2',
    'chapter_list': []
  },
];
